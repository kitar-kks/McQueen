...
def main(argv):
    ...
    config_path = darknet_path + "yolov4-tiny-obj.cfg"
    weight_path = "yolov4-tiny-obj_last.weights"
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            for detection in detections:
                ...
                #ss = cv2.addWeighted(image,1, heading_image, 0.5, 0)
            ss = cv2.addWeighted(image,1, heading_image, 0.5, 0)
            cv2.imshow("ZED", ss)
            #cv2.imshow("heading line",heading_image)
            #cv2.imshow("ZED", images_total)
            key = cv2.waitKey(5)
